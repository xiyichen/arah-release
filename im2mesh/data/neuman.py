...
class NEUMANDataset(data.Dataset):
    ''' NeuMan dataset class.
    '''
    ...
    def __init__(self, dataset_folder,
        ...
        for subject in subjects:
            ...
            for cam_idx, cam_name in enumerate(cam_names):
                ...
                frames = [int(s.split('/')[-1].split('.')[0]) for s in img_files]
    ...
    def world_to_cam(self, world_coord, R, t):
    ...
    def __getitem__(self, idx):
        ...
        K = np.array(self.cameras[cam_name]['K'][str(self.frames[cam_name][idx])], dtype=np.float32)
        dist = np.array(self.cameras[cam_name]['D'][str(self.frames[cam_name][idx])], dtype=np.float32).ravel()
        R = np.array(self.cameras[cam_name]['R'][str(self.frames[cam_name][idx])], np.float32)
        cam_trans = np.array(self.cameras[cam_name]['T'][str(self.frames[cam_name][idx])], np.float32).ravel()
